...
void ball_detect()
{
  ...
  msg.size = circles.size();     // adjust the size of message. (*the size of message is varying depending on how many
  ...
  ball_list.pose.position.x =
      0;  // the transformation between the frame and camera data, just set it (0,0,0,0,0,0) for (x,y,z,roll,pitch,yaw)
  ...
  for (int k = 0; k < circles.size(); k++)
  {
    ...
    circle(buffer, center, r, Scalar(0, 0, 255), 10);  // draw a circle on 'frame' based on the information given,   r =
                                                       // radius, Scalar(0,0,255) means color, 10 means lineWidth

    // cy = 3.839*(exp(-0.03284*cy))+1.245*(exp(-0.00554*cy));   //convert the position of the ball in camera coordinate
    // to the position in base coordinate. It is related to the calibration process. You shoould modify this. cx =
    // (0.002667*cy+0.0003)*cx-(0.9275*cy+0.114);
